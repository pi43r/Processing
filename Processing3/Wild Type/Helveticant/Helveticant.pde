...
void setup() {
  textSize = 150; //Needed for performance, bc the ant only checks its location at textbox
  size(1200, 700 ); //Set your screen size, bigger size = lower framerate
  smooth();
  ...
  f = loadFont("Helvetica-Bold-150.vlw");
...
void draw() {
  ...
  if (myText.length() > 0) {
    ...
    pg.text(myText,width/2-10,height/2);
    pg.loadPixels();
  }
  else{
